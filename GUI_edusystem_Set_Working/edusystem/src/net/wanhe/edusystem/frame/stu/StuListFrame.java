    ...
    public StuListFrame() {
        ...
        button_2.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                dispose();
                //通过构造方法将学生信息传递给修改窗口
                StuUpdateFrame stuUpdateFrame = new StuUpdateFrame(stu);
                stuUpdateFrame.setVisible(true);
...
//        };
        ...
        Object[][] datas = new Object[stus.length][5];
        for(int i = 0;i < datas.length;i++){
            datas[i][0] = stus[i].getNum();
            datas[i][1] = stus[i].getName();
            datas[i][2] = stus[i].getAge();
            datas[i][3] = stus[i].getPhone();
            datas[i][4] = stus[i].getAddress();
        }
        //定义一个表格模型
